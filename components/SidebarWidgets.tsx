...
// Site-wide sidebar rail: Must Read / Most Popular / state filter / 8th Pay
// Commission blurb. Always built from the full site circular list — shared
// identically by the homepage and every /section/[slug] page, never
// filtered down to whatever subset that page happens to be showing.
export default function SidebarWidgets({ circulars }: { circulars: Circular[] }) {
  const mustRead = circulars.filter((c) => c.isFeatured).slice(0, 4);
  const mostPopular = [...circulars].sort((a, b) => b.viewCount - a.viewCount).slice(0, 8);
